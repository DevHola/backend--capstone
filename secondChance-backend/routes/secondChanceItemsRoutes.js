...
router.post('/', upload.single('file'), async(req, res,next) => {
    try {
        ...
        if (lastItem.length > 0) {
            ...
            secondChanceItem.id = "1"; // Start from 1 if no items exist
        }
        const date_added = Math.floor(new Date().getTime() / 1000);
        secondChanceItem.date_added = date_added;
        Item = await collection.insertOne(secondChanceItem);
        console.log(secondChanceItem)
        res.status(201).json({ id: Item.insertedId, ...secondChanceItem });
    } catch (e) {
        next(e);
    }
});
...
router.delete('/:id', async(req, res,next) => {
    try {
        ...
        await collection.deleteOne({ id: secondChanceItem.id });
